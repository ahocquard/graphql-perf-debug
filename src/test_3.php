...
$attributeCodes = array_map(function ($index) { return AttributeCode::createFromString("a$index"); }, range(0, 599));
$attributeCodesChunk = array_chunk($attributeCodes, 100);

$i = 0;
foreach ($familyCodes as $codeString) {
    $familyRepository->add(createFamily($codeString, $attributeCodesChunk[$i]));
    $i++;
    if (!isset($attributeCodesChunk[$i])) {
        $i = 0;
    }
}
